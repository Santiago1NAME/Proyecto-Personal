...
const Input: React.FC<Props> = ({ title, name, type, required, onChange }) => {
  return (
    <div className="relative w-full mt-3 mb-3">
        <input
          id={ name }
          name={name}
          className="block w-full border-solid border-2 border-[#f2f2f2] rounded-sm p-2 focus:outline-none focus:ring-2 focus:ring-cyan-300 peer"
          type={ type }
          placeholder=" "
          onChange={onChange}
        />
